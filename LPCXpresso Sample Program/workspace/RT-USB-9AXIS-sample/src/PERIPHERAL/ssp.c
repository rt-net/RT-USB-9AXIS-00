...
void SSPInit( void )
{
	 uint8_t i, Dummy=Dummy;
	  ...
	  LPC_SYSCON->SSPCLKDIV = 0x0f;                 /* Divided by 2 */
      ...
      LPC_GPIO0->DIR  |= (0x1<<2);     //Data Direction(1 output , 0 input)
...
void readWhoAmI()
{
	...
	SSPSend( &send_data[0], 1 );
	SSPReceive( &receive_data[0], 2 );

	LPC_GPIO0->DATA |= (0x1<<2);

	myPrintfUSB("whoAmI %d %d %d \n",receive_data[0],receive_data[1],receive_data[2] );
}
#endif
